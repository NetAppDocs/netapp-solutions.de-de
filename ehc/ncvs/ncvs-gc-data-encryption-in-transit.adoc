...
Google Cloud verschlüsselt den Datenverkehr auf Netzwerkebene wie in beschrieben https://["Verschlüsselung während der Übertragung"^] In der Google-Dokumentation. Wie im Abschnitt „Cloud Volumes Services Architecture“ erwähnt, wird Cloud Volumes Service aus einem von NetApp gesteuerten PSA Producer-Projekt bereitgestellt.
...
Für NFSv4.1 bietet CVS-Performance Kerberos-Authentifizierung wie in beschrieben https://["RFC7530"^]. Sie können Kerberos auf Volume-Basis aktivieren.
...
As per RedHat https://["Allgemeine Mount-Optionen"^] Dokumentation:
