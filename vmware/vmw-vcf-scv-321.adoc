...
[%collapsible%open]
====
Mit dieser Lösung soll die Datensicherung von Virtual Machines demonstriert werden, die in VMware vSphere, VCF VI-Workload-Domänen oder VCF-Management-Domänen ausgeführt werden. Virtual Machines in dieser Lösung werden auf NFS-Datenspeichern gehostet, die von NetApp ONTAP bereitgestellt werden. Bei dieser Lösung wird vorausgesetzt, dass die folgenden Komponenten konfiguriert und einsatzbereit sind:
