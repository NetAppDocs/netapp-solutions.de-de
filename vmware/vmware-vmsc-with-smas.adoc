...
* link:vmware_vcf_asa_supp_mgmt_iscsi.html#deployment-steps["VMware vSphere-Hosts, die mit Dual-Storage Fabric (zwei HBAs oder Dual-VLAN für iSCSI) pro Host bereitgestellt werden"].
...
* link:vmware_vcf_asa_supp_mgmt_iscsi.html#deployment-steps["Storage VM und LIFs sind verfügbar"]
