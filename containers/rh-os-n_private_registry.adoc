...
NOTE: Für den Trident Protect ist eine Registrierung erforderlich, um die von Astra-Containern benötigten Images zu hosten. Im folgenden Abschnitt werden die Schritte zum Einrichten einer privaten Registrierung auf dem Red hat OpenShift-Cluster und zum Drücken der Images beschrieben, die zur Unterstützung der Installation von Trident Protect erforderlich sind.
