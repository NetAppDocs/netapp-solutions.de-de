...
====
'''
Sidebar: Sidebar permalink: hyperv/hyperv-smis.html Keywords: hyperv, Hyper-V, SMI-S, SCVMM, NetApp, Virtualisierung, iscsi, SMB,microsoft Zusammenfassung: "Dieser Artikel dokumentiert, wie NetApp SMI-S Provider installiert und verwaltet und Speicher mit Microsoft SCVMM bereitgestellt wird. „ ---

= NetApp SMI-S Provider für Microsoft Hyper-V :hardbreaks: :Nofooter: :Icons: Font :linkattrs: :Imagesdir: ../media/
...
==== Hauptmerkmale und -Vorteile: • Erkennung und Management von Speichersystemen: Erkennt Speichersysteme mit ONTAP-Software und stellt Tools für ein effektives Management bereit. • Vereinfachte Administration: Vereinfacht das Management der Storage-Systeme über eine einzige, befehlsbasierte Schnittstelle. • Monitoring und Reporting: Bietet robuste Monitoring- und Berichtsfunktionen zur Überwachung von Storage-Elementen und ihrer Performance.

image:hyperv-smis-image1.png["NetApp SMI-S mit Hyper-V Architektur"]

== machen Sie sich bereit für die Bereitstellung von SMI-S Provider
...
=== Hardwareanforderungen für virtuelle SMI-S-Maschinen die folgende Tabelle zeigt die Hardwareanforderungen für die VM des SMI-S-Providers.
...
=== SMI-S-Provider mit SCVMM verbinden
...
=== iSCSI-Speicher . Wählen Sie in der SCVMM-Konsole Struktur > Speicher , klicken Sie mit der rechten Maustaste auf *Klassifizierungen und Pools* und wählen Sie *logische Einheit erstellen*. Wählen Sie den Speicherpool und die Klassifizierung aus, und geben Sie den Namen, die Beschreibung, die Größe und die Hostgruppe für die Logikeinheit ein. image:hyperv-smis-image9.png["Logikeinheit-Speicher erstellen"]

=== SMB-Storage . Wählen Sie *Struktur* > *Speicher* > Rechtsklick *Dateiserver* und wählen Sie *Dateifreigabe erstellen*, wählen Sie *Dateiserver*, geben Sie den Namen, den Speichertyp, den Speicherpool und die Klassifizierung ein. image:hyperv-smis-image10.png["Erstellen Sie Dateifreigaben"]

== Protokolle und Traces Sie können konfigurieren, wie SMI-S Provider Protokolle und Trace-Dateien verwaltet, wie beispielsweise die Meldungsebenen und das Verzeichnis, in dem Protokolle gespeichert werden, angeben. Sie geben auch die Komponenten an, die verfolgt werden sollen, das Ziel, auf das Trace-Meldungen geschrieben werden, die Tracing-Ebene und den Speicherort der Trace-Datei.

==== Protokolleinstellung standardmäßig werden alle Systemmeldungen protokolliert und die Systemmeldungsprotokolle befinden sich im Protokollverzeichnis des Verzeichnisses, in dem NetApp SMI-S Provider installiert ist. Sie können den Speicherort und die Ebene der Systemmeldungen ändern, die in das CIM-Serverprotokoll geschrieben werden.
...
=====
==== Einstellung der Kurve image:hyperv-smis-image12.png["Trace-Einstellung"]
