...
summary: Auf dieser Seite werden die Aufgaben beschrieben, die Sie für die Bereitstellung von Kubeflow in Ihrem Kubernetes-Cluster abschließen müssen. 
---
= Kubeflow Deployment
:hardbreaks:
:allow-uri-read: 
...
. Sie haben bereits einen funktionierenden Kubernetes-Cluster und führen eine Version von Kubernetes aus, die von Kubeflow unterstützt wird. Eine Liste der unterstützten Versionen finden Sie im https://www.kubeflow.org/docs/started/getting-started/["Offizielle Dokumentation von Kubeflow"^].
. Sie haben NetApp Trident bereits in Ihrem Kubernetes Cluster installiert und konfiguriert, wie in beschrieben link:aicp_netapp_trident_deployment_and_configuration_overview.html["Implementierung und Konfiguration von Trident"].
...
NOTE: Alternativ können Sie Kubeflow manuell bereitstellen, indem Sie dem folgen https://www.kubeflow.org/docs/started/getting-started/["Installationsanweisungen"^] In der offiziellen Kubeflow Dokumentation

. Implementieren Sie Kubeflow in Ihrem Cluster, indem Sie den folgen https://github.com/NVIDIA/deepops/blob/master/docs/k8s-cluster/kubeflow.md["Anweisungen zur Bereitstellung von Kubeflow"^] Auf der NVIDIA DeepOps GitHub Website.
. Beachten Sie unten die URL des Kubeflow Dashboard, die vom DeepOps Kubeflow Bereitstellungs-Tool ausgegeben wird.
+
....
...
....
. Vergewissern Sie sich, dass alle im Kubeflow Namespace bereitgestellten Pods A zeigen `STATUS` Von `Running` Und vergewissern Sie sich, dass sich keine im Namespace bereitgestellten Komponenten in einem Fehlerzustand befinden. Es kann mehrere Minuten dauern, bis alle Pods beginnen.
+
....
...
....
. Rufen Sie in Ihrem Webbrowser das zentrale Kubeflow Dashboard auf, indem Sie zur URL navigieren, die Sie in Schritt 2 angegeben haben.
+
Der Standardbenutzername lautet `admin@kubeflow.org`, Und das Standardpasswort lautet `12341234`. Um weitere Benutzer zu erstellen, befolgen Sie die Anweisungen im https://www.kubeflow.org/docs/components/multi-tenancy/["Offizielle Dokumentation von Kubeflow"^].



image:aicp_image8.png["Fehler: Fehlendes Grafikbild"]

link:aicp_example_kubeflow_operations_and_tasks_overview.html["Weiter: Beispiel Kubeflow Operations und Tasks."]
