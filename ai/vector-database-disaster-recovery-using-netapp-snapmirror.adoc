...
. Skalierbarkeit: Die Hybrid-Cloud-Lösung von NetApp bietet die Möglichkeit, Ressourcen nach Bedarf zu skalieren. Sie können On-Premises-Ressourcen für regelmäßige, vorhersehbare Workloads und Cloud-Ressourcen wie Amazon FSX ONTAP für NetApp ONTAP und Google Cloud NetApp Volume (GCNV) für Spitzenlasten oder unerwartete Workloads nutzen.
