...
image:ontap-cyber-vault-connections.png["Überprüfen Sie, ob keine Verbindungen zum Cyber-Vault bestehen"]

Dieser Screenshot zeigt, dass es keine Möglichkeit gibt, die Snapshots zu manipulieren.

image:ontap-cyber-vault-tamperproof-snapshots.png["Überprüfen Sie, ob Snapshot Kopien nicht manipuliert werden können"]
