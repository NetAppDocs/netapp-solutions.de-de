...
In dieser Dokumentation wird die vereinfachte Implementierung von Oracle-Datenbanken in einer SAN-Umgebung mit ASA Systemen mithilfe von Ansible-Automatisierung demonstriert. Die Oracle Datenbank wird in einer eigenständigen Neustartkonfiguration mit iSCSI-Protokoll für den Datenzugriff und Oracle ASM für das Storage-Management auf dem ASA Storage Array bereitgestellt. Wir zeigen außerdem, wie das UI-Tool NetApp SnapCenter zum Sichern, Wiederherstellen und Klonen einer Oracle Datenbank für Entwicklung/Tests oder andere Anwendungsfälle für den Storage-effizienten Datenbankbetrieb in NetApp ASA Systemen verwendet wird.
...
image::automation_ora_asa_iscsi_archit.png[Dieses Bild zeigt ein detailliertes Bild der Oracle-Implementierungskonfiguration in AWS Public Cloud mit iSCSI und ASM.]
...
. Führen Sie alle Playbooks in einer kombinierten Ausführung aus.
+
[source, cli]
----
