...
summary: Die Lösung bietet eine Übersicht und Details zur schnellen Wiederherstellung von Oracle VLDB, das auf Google Compute Engine VM mit NFS-Mount auf dem Google Cloud NetApp Volumes-Speicherpool bereitgestellt wird, um eine Standby-Datenbankkopie bereitzustellen, die ständig inkrementell über RMAN zusammengeführt wird. 
---
= TR-5007: Optimieren Sie Oracle VLDB Backup & Recovery mit inkrementeller Zusammenführung im Server auf Google Cloud NetApp Volumes
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Die Wiederherstellung einer sehr großen Datenbank (VLDB) in Oracle mit dem Backup-Tool Oracle Recovery Manager (RMAN) kann eine große Herausforderung darstellen. Der Datenbankwiederherstellungsprozess von Backup-Medien im Fehlerfall kann zeitaufwändig sein, wodurch die Datenbank-Recovery verzögert wird und möglicherweise Ihr Service Level Agreement (SLA) erheblich beeinträchtigt wird. Für Oracle VLDBs in der Cloud verschärft sich die Situation noch weiter, da RMAN-Backups aus Kostengründen in der Regel in Objektspeicher gestreamt werden. Daher ist die Wiederherstellung einer VLDB nicht sinnvoll, wenn ein SLA erforderlich ist. Ab Version 10g hat Oracle eine RMAN-Funktion eingeführt, mit der Benutzer gestaffelte Image-Kopien der Oracle-Datenbankdatendateien auf zusätzlichem Festplattenspeicher auf dem DB-Server-Host erstellen können. Diese Image-Kopien können mit RMAN in benutzerdefinierten Intervallen inkrementell aktualisiert werden. Bei einem Ausfall kann der Datenbankadministrator (DBA) die Oracle-Datenbank schnell von den fehlerhaften Medien auf die Image-Kopie umschalten, sodass keine vollständige Wiederherstellung der Datenbankmedien erforderlich ist. Das Ergebnis ist ein deutlich verbesserter SLA, der allerdings mit der Verdopplung des erforderlichen Datenbank-Storage verbunden ist.

Wenn Sie Wert auf ein SLA für Ihre VLDB legen und die Oracle-Datenbank in eine öffentliche Cloud wie die Google Cloud Platform (GCP) migrieren möchten, können Sie eine ähnliche Datenbankschutzstruktur mit Ressourcen wie Google Cloud NetApp Volumes (GCNV) für die Bereitstellung Ihrer Standby-Datenbank-Image-Kopie einrichten. GCNV ist ein vollständig verwalteter, leistungsstarker Dateispeicherdienst von Google, der die Migration und Ausführung anspruchsvoller Unternehmensanwendungen wie der Oracle-Datenbank in Google Cloud einfacher, sicherer und kostengünstiger macht. Die verschiedenen Service-Levels ermöglichen die flexible Kontrolle der Speicherkosten bei gleichzeitiger Erreichung eines RPO von nahezu null und eines überschaubaren RTO. In dieser Dokumentation zeigen wir, wie Sie ein NFS-Dateisystem aus einem GCNV-Speicherpool bereitstellen und exportieren, um es auf einem Oracle-Datenbankserver bereitzustellen und so eine Standby-Datenbankkopie für eine schnelle Wiederherstellung im Falle eines primären Speicherausfalls bereitzustellen.

Diese Lösung eignet sich für folgende Anwendungsfälle:

* Sicherung der Imagekopie von Oracle-Datenbankdatendateien über RMAN auf ein serverinternes NFS-Dateisystem in einem Google Cloud NetApp Volumes-Speicherpool.
* Schnelle Wiederherstellung einer Oracle VLDB im Falle eines Fehlers auf derselben oder einer Standby-Google Compute Engine-VM.
* Klonen Sie in wenigen Minuten eine Oracle-Datenbankkopie auf einem Klonhost für andere Anwendungsfälle aus der Staging-DB-Imagekopie.
...
* Ein DBA, der Oracle VLDB in der Google Cloud Platform einrichtet und verwaltet.
* Ein Datenbanklösungsarchitekt, der Oracle-Workloads in der Google Cloud Platform testet.
* Ein Speicheradministrator, der in GCNV-Speicherpools bereitgestellte Oracle-Datenbanken verwaltet.
* Ein Anwendungsbesitzer, der Oracle-Datenbanken in der Google Cloud Platform einrichten möchte.
...
Das Testen und Validieren dieser Lösung wurde in Google Cloud NetApp Volumes-Speicherpools und Google Compute Engine-VMs durchgeführt, die möglicherweise nicht der endgültigen Bereitstellungsumgebung entsprechen. Weitere Informationen finden Sie im Abschnitt <<Wichtige Faktoren für die Implementierung>>.



=== Der Netapp Architektur Sind

image:gcp_ora_gcnv_inc_merge_archit.png["Dieses Bild bietet eine detaillierte Darstellung der inkrementellen Zusammenführungsimplementierung von Oracle VLDB in der Google Cloud mit GCNV."]
...
| Google Cloud NetApp Volumes-Speicherpools | Aktuelle Dienste von Google | 2 TiB Speicherpool mit Premium-Service, 2 TiB Speicherpool mit Standard-Service 


| Google Compute Engine VM für DB-Server | N1 (4 vcpus, 15 GB Arbeitsspeicher) | 2 VMs, eine als primärer DB-Server und die andere als Standby- oder Klon-DB-Server 
...
* *GCNV-Speicherpool-Servicelevel.* Der GCNV-Speicherpool bietet vier Servicelevel: Standard, Premium, Extreme und Flex. Standardmäßig gilt für ein im Speicherpool erstelltes Volume eine automatische QoS, die den Durchsatz des Volumes basierend auf seiner Größe und dem Servicelevel begrenzt. Einzige Ausnahme: Volumes, die aus dem Flex-Speicherpool zugewiesen werden, teilen sich den für den Speicherpool verfügbaren Durchsatz.
* *Speicherlimit für Oracle VLDB-Backups im Server für inkrementelle RMAN-Zusammenführung.* In unserem Test und unserer Validierung wird ein NFS-Volume für inkrementelle Oracle-Backups und -Zusammenführungen aus einem einzelnen GCNV-Speicherpool mit Standard-Servicelevel zugewiesen, der eine Kapazitätsgrenze von 200 TiB hat. Für Implementierungen über diesen Grenzwert hinaus können mehrere GCNV-Speicherpools parallel mit mehreren NFS-Mount-Punkten implementiert werden, um eine höhere Kapazität zu erreichen.
* *Oracle-Wiederherstellung durch inkrementelles RMAN-Merge.* Die inkrementelle Sicherung und Zusammenführung im Server durch RMAN erfolgt in der Regel in einer benutzerdefinierten Häufigkeit, basierend auf Ihren RTO- und RPO-Zielen. Bei einem vollständigen Verlust des primären Datenbankdateispeichers und/oder der archivierten Protokolle kann es zu Datenverlust kommen. Die Oracle-Datenbank kann bis zum letzten verfügbaren inkrementellen Backup aus der serverinternen Datenbank-Backup-Imagekopie wiederhergestellt werden. Um Datenverluste zu minimieren, muss der Oracle-Flash-Recovery-Bereich auf dem GCNV-NFS-Backup-Mount-Punkt eingerichtet werden. Archivierte Protokolle werden zusammen mit der Datenbank-Imagekopie auf demselben NFS-Mount gesichert, um im Fehlerfall die Datenbank auf das letzte archivierte Protokoll zurückzusetzen. Dadurch kann ein RPO von nahezu Null erreicht werden.
* *Ausführen einer Oracle-VLDB mit NFS-Image-Backup im GCNV-Speicherpool.* Eine Neuzuweisung des Datenbank-Volumes von Standard auf Premium oder Extreme wird derzeit nicht unterstützt. Die Neuzuweisung von Service-Levels von Premium auf Extreme wird jedoch unterstützt. Um die Datenbank nach einem Ausfall über einen längeren Zeitraum mit Image-Kopie auszuführen, wird empfohlen, Volumes aus dem Premium-Speicherpool für die Sicherung der Datenbank-Image-Kopie zuzuweisen.
* *Google Compute Engine VMs.* Für den Test und die Validierung verwendeten wir GCE-VM-Instanzen vom Typ N1 als Oracle-Datenbankserver. Es gibt andere GCE-VM-Typen, die möglicherweise besser optimiert und für Datenbank-Workloads besser geeignet sind. Passen Sie die VM-Größe entsprechend der Anzahl der vCPUs und der RAM-Menge an die tatsächlichen Workload-Anforderungen an.
* *dNFS-Konfiguration.* dNFS ist in den Oracle-Kernel integriert und steigert die Leistung von Oracle-Datenbanken deutlich, wenn Oracle auf NFS-Speichern bereitgestellt wird. dNFS ist in der Oracle-Binärdatei enthalten, aber standardmäßig nicht aktiviert. Sie sollte für jede Oracle Database-Bereitstellung auf NFS aktiviert werden. Für die Bereitstellung mehrerer GCNV-Speicherpools für eine VLDB müssen dNFS-Mehrpfade zu verschiedenen GCNV-Speicherpools ordnungsgemäß konfiguriert werden.
...
Die Lösung setzt voraus, dass eine Oracle VLDB bereits in der Google Cloud-Umgebung innerhalb einer VPC bereitgestellt ist. Die Oracle VLDB kann entweder auf dem GCNV-Speicher oder auf anderen Speicheroptionen innerhalb des Google Cloud-Ökosystems ausgeführt werden. Der folgende Abschnitt beschreibt Schritt-für-Schritt-Anleitungen zum Einrichten eines Oracle VLDB-Image-Backups und der inkrementellen Zusammenführung per RMAN in ein NFS-Dateisystem, das aus einem GCNV-Speicherpool bereitgestellt wird. Der primäre Speicher der Oracle VLDB befindet sich ebenfalls in einem anderen GCNV-Speicherpool.
...
. Ein GCP-Konto wurde eingerichtet und die erforderliche VPC- und Netzwerkkonfiguration wurde innerhalb eines Projekts im Google Cloud-Konto erstellt.
. Befolgen Sie in der Google Cloud-Portalkonsole die Dokumentation link:https://cloud.google.com/netapp/volumes/docs/configure-and-use/storage-pools/create-storage-pool["Erstellen Sie einen Speicherpool"^] um einen GCNV-Speicherpool mit Standarddienst zu erstellen, wie unten gezeigt.
+
image:gcp_ora_gcnv_inc_merge_pool_01.png["Dieses Bild zeigt den GCNV-Speicherpool-Bildschirm von der Google Cloud-Portalkonsole"]

. Befolgen Sie in der Google Cloud-Portalkonsole die Dokumentation link:https://cloud.google.com/netapp/volumes/docs/configure-and-use/volumes/create-volume["Erstellen eines Volumes"^] um ein Volume für die serverinterne Oracle VLDB-Imagesicherung zu erstellen. In dieser Lösungsdemo wird der Oracle VLDB-Primärspeicher, wie unten gezeigt, ebenfalls auf GCNV gehostet.
+
image:gcp_ora_gcnv_inc_merge_vol_02.png["Dieses Bild zeigt den GCNV-Speichervolumes-Bildschirm von der Google Cloud-Portalkonsole"]
...
=== Mounten Sie ein NFS-Volume von GCNV auf dem primären Oracle VLDB-Server

[%collapsible%open]
====
Das für die Oracle VLDB-Sicherung im Server bereitgestellte GCNV-Volume sollte sich für eine bessere Leistung idealerweise in derselben Google Cloud-Zone wie der primäre DB-Server und der primäre Speicher befinden.

. Rufen Sie den NFS-Exportpfad des GCNV-Volumes aus der Google Cloud-Konsole ab.
+
image:gcp_ora_gcnv_inc_merge_vol_03.png["Dieses Bild zeigt den GCNV-Speichervolumes-Bildschirm von der Google Cloud-Portalkonsole"]

. Melden Sie sich beim primären Oracle VLDB-Server an. Mounten Sie als Root das aus dem GCNV-Speicher exportierte NFS-Volume. Ändern Sie bei Bedarf die IP-Adresse und den Dateipfad Ihres NFS-Servers.
+
[source, cli]
----
sudo mkdir /nfsgcnv
...
----
. Ändern Sie den Mount-Point-Eigentümer in oracle:oisntall oder ändern Sie ihn nach Bedarf in Ihren Oracle-Benutzernamen und Ihre primäre Gruppe.
+
[source, cli]
----
sudo chown oracle:oinstall /nfsgcnv
----
. Fügen Sie die NFS-Einbindung zu /etc/fstab hinzu.
+
[source, cli]
----
sudo vi /etc/fstab
...
=== Einrichten der inkrementellen Zusammenführung von Oracle RMAN mit einer Image-Kopie auf GCNV für die serverinterne VLDB-Sicherung

[%collapsible%open]
====
Die inkrementelle Zusammenführung von RMAN aktualisiert die Imagekopie der Datendateien der Staging-Datenbank kontinuierlich in jedem inkrementellen Sicherungs-/Zusammenführungsintervall. Die Image-Kopie der Datenbanksicherung ist so aktuell wie die Häufigkeit, in der Sie die inkrementelle Sicherung/Zusammenführung durchführen. Berücksichtigen Sie also die Datenbankleistung, Ihre RTO- und RPO-Ziele bei der Entscheidung über die Häufigkeit der inkrementellen Backups und Merge von RMAN.

. Melden Sie sich beim primären oracle VLDB-Server als oracle-Benutzer an.
. Für diese Lösungsvalidierung wird die primäre Datenbank als eigenständiger Server mit Grid-Infrastruktur eingerichtet. Starten Sie die Konfiguration wie unten gezeigt neu. Der primäre Speicher wird im GCNV-Speicherpool mit Premium-Service gehostet. Auch dies ist keine Voraussetzung, und der primäre Speicher kann auf jeder anderen Speicheroption in der Google Cloud liegen.
+
....
...
....
. Erstellen Sie unter dem Einhängepunkt /nfsgcnv ein Oracopy-Verzeichnis, um Imagekopien von Oracle-Datendateien und ein Archlog-Verzeichnis für den Oracle-Flash-Wiederherstellungsbereich zu speichern.
+
[source, cli]
----
...
----
. Melden Sie sich über sqlplus bei der Oracle Datenbank an, aktivieren Sie die Nachverfolgung von Blockänderungen für schnellere inkrementelle Backups und ändern Sie den Oracle Flash Recovery-Bereich in GCNV NFS-Mount, wenn er sich derzeit im Primärspeicher befindet. Dadurch können die automatische Sicherung der RMAN-Standardsteuerdatei/SP-Datei und der archivierten Protokolle auf dem GCNV-NFS-Mount gesichert werden, um die Datenbank auf das letzte verfügbare Archivprotokoll wiederherzustellen.
+
[source, cli]
----
sqlplus / as sysdba
...
....
. Melden Sie sich auf dem primären Oracle VLDB-Server lokal als oracle-Benutzer mit oder ohne RMAN-Katalog bei RMAN an. In dieser Demo stellen wir keine Verbindung zu einem RMAN-Katalog her.
+
....
...
....
. Führen Sie nach dem Backup eine Datenbank-Image-Kopie auf, um zu beobachten, dass im GCNV NFS-Bereitstellungspunkt eine Datenbank-Image-Kopie erstellt wurde.
+
....
...
....
. Melden Sie das Schema über die Eingabeaufforderung von Oracle RMAN, um zu beobachten, dass sich die aktuellen VLDB-Datendateien auf dem primären Speicher befinden.
+
....
...
....
. Führen Sie das RMAN-Sicherungsskript in gewünschten Intervallen über ein einfaches UNIX-Shell-Skript aus Crontab aus.
+
....
...
Damit ist die Einrichtung einer Oracle VLDB-Standby-Image-Kopiesicherung und -Zusammenführung auf einem serverinternen NFS-Mount vom GCNV-Speicher abgeschlossen.

====


=== Schnelle Wiederherstellung von Oracle VLDB mithilfe einer Backup-Image-Kopie



==== Vollständige Wiederherstellung bei Verlust oder Beschädigung von Oracle-Datendateien

[%collapsible%open]
====
Im Falle eines Fehlers aufgrund eines primären Speicherproblems wie Datenverlust oder -beschädigung kann die Datenbank schnell auf eine Image-Kopie auf dem GCNV NFS-Mount umgeschaltet und ohne Datenbankwiederherstellung in den aktuellen Zustand zurückversetzt werden. Durch die Eliminierung der Medienwiederherstellung wird die Datenbank-Recovery für ein VLDB erheblich beschleunigt. Dieser Anwendungsfall setzt voraus, dass der Oracle VLDB DB-Server intakt ist und dass die Datenbanksteuerungsdatei, die archivierten und die aktuellen Protokolle für die Recovery verfügbar sind.

. Melden Sie sich als Oracle-Benutzer beim primären VLDB-Server-Host an und erstellen Sie vor der Umstellung eine Testtabelle.
+
....
...
....
. Fügen Sie eine Zeile in die Testtabelle ein. Die Zeile wurde nach der letzten inkrementellen Sicherung und Zusammenführung eingefügt, und ihre Daten sind nicht im Sicherungssatz enthalten. Die nachfolgende Demonstration zeigt, dass die Testzeile nach der vollständigen Wiederherstellung der Datenbank wiederhergestellt wird.
+
....
...
....
. Simulieren Sie einen Fehler, indem Sie die Datenbank herunterfahren.
+
....
...
....
. Stellen Sie als Oracle-Benutzer über RMAN eine Verbindung zur Oracle-Datenbank her, um zur zu kopierenden Datenbank zu wechseln.
+
....
...
....
. Überprüfen Sie die Datenbankstruktur von sqlplus nach der Wiederherstellung, um zu beobachten, dass alle VLDB-Datendateien mit Ausnahme von Steuerungs-, Temp- und aktuellen Protokolldateien nun auf das GCNV NFS-Dateisystem kopiert werden.
+
....
...
....
. Überprüfen Sie in SQL Plus, ob die Testzeile, die wir nach der letzten inkrementellen Sicherung und vor dem Umschalten auf die Image-Sicherungskopie inaktiviert haben, wiederhergestellt wurde.
+
....
...
==== Setzen Sie die Oracle VLDB aus der Sicherungskopie zurück in den primären Speicher

[%collapsible%open]
====
Nachdem der Speicherfehler im Primärspeicher behoben wurde, kann die Datenbank problemlos und mit minimaler Ausfallzeit wieder auf den Primärspeicher zurückgesetzt werden. Nachfolgend finden Sie eine Schritt-für-Schritt-Anleitung.

. Überprüfen Sie mithilfe von RMAN, ob sich das DB-Schema derzeit im GCNV-Sicherungsspeicher befindet.
+
....
...
....
. Nachdem die primäre Datenbank auf eine Imagekopie umgestellt wurde, ist die primäre Datenbank jetzt eine Kopie im RMAN-Datensatz.
+
....
...
....
. Löschen Sie die jetzt veralteten Datendateien auf dem Primärspeicher, falls vorhanden, um Speicherplatz freizugeben und/oder Datensätze zu löschen.
+
....
...
....
. Überarbeiten Sie das RMAN-Backup- und Merge-Skript, um auf den Primärspeicher und in unserem Testfall auf die ASM-Datenträgergruppe +DATA in der RMAN-Kanaldefinition zu verweisen. Führen Sie das überarbeitete Backup-Skript aus, um eine neue Image-Kopie auf dem Primärspeicher zu erstellen.
+
....
...
....
. Überprüfen Sie, ob jetzt eine neue VLDB-Imagekopie auf dem primären Speicher erstellt wurde.
+
....
...
....
. Fügen Sie eine neue Testzeile in die Testtabelle ein und überprüfen Sie, ob durch die Rückkehr zur Imagekopie auf dem Primärspeicher alle Transaktionen wiederhergestellt werden.
+
....
...
....
. Fahren Sie die Datenbank von RMAN herunter, um zurück zur Backup-Image-Kopie auf dem primären Speicher zu wechseln.
+
....
...
....
. Überprüfen Sie, ob sich die Datenbank jetzt wieder im primären Speicher befindet.
+
....
...
....
. Überprüfen Sie von SQLPlus aus, ob alle Transaktionen in der Testtabelle wiederhergestellt wurden.
+
....
...
....
. Löschen Sie abschließend die veraltete DB-Image-Kopie auf dem GCNV-Sicherungsspeicher und erstellen Sie ein neues Backup-Image auf dem GCNV-Sicherungsspeicher, um den Zyklus abzuschließen.


====


==== Point-in-Time-Wiederherstellung mit Verlust aller Oracle-Dateien

[%collapsible%open]
====
Bei einem Ausfall des primären Speichers und/oder des primären DB-Server-Hosts gehen alle Transaktionen verloren, die nicht in den archivierten Protokolldateien enthalten sind. Allerdings ist Ihre Backup-Image-Kopie für Oracle-Datenbanken, die auf dem GCNV NFS-Dateisystem verfügbar ist, äußerst praktisch. Mithilfe der Backup-Image-Kopie können Sie die primäre Datenbank schnell auf demselben primären Datenbankserver oder, falls verfügbar, auf einem Standby-Datenbankserver wiederherstellen. In diesem Abschnitt wird die Wiederherstellung auf dem primären DB-Server Schritt für Schritt erläutert. Die Wiederherstellung auf einem neuen Standby-DB-Server ist nahezu identisch. Das Datenbank-Backup-Image muss jedoch auf dem Standby-DB-Server mit demselben installierten und konfigurierten Oracle-Software-Stack bereitgestellt werden.

. Fügen Sie eine Zeile in die Testtabelle ein, die wir zuvor für die Validierung der Oracle VLDB-Point-in-Time-Wiederherstellung erstellt haben.
+
....
...
....
. Archivieren Sie die aktuelle Protokolldatei, sodass die Transaktion in eine archivierte Protokolldatei übertragen wird.
+
....
...
....
. Fahren Sie den primären VLDB-Server-Host herunter, um einen Totalausfall von Speicher und DB-Server-Host zu simulieren.
. Wenn die Wiederherstellung auf einem Standby-DB-Server durchgeführt wird, stellen Sie sicher, dass es sich um dasselbe Betriebssystem, dieselbe Version und denselben Betriebssystemkernel handelt wie beim primären DB-Server-Host. Außerdem wurden die gleiche Version und Patches von Oracle auf Standby-DB-Server mit rein softwarebasierter Option installiert und konfiguriert.
. Konfigurieren Sie außerdem die Oracle-Umgebung auf dem Standby-DB-Server ähnlich wie auf dem primären DB-Server, z. B. oratab, und den Oracle-Benutzer .bash_profile usw. Es empfiehlt sich, diese Dateien auf dem GCNV-NFS-Mount-Punkt zu sichern.
. Die Backup-Image-Kopie der Oracle-Datenbank auf dem GCNV NFS-Dateisystem wird dann zur Wiederherstellung auf dem Standby-DB-Server gemountet. Die folgenden Verfahren zeigen die Prozessdetails.
+
Erstellen Sie als Administratorbenutzer den Einhängepunkt.
...
----
+
Mounten Sie als Administratorbenutzer das NFS-Volume, auf dem die Oracle VLDB-Sicherungsimagekopie gespeichert ist.
...
----
. Überprüfen Sie die verfügbaren archivierten Oracle-Protokolle auf dem GCNV-NFS-Mount für die Wiederherstellung, und notieren Sie sich die letzte Protokollsequenznummer. In unserem Testfall waren es 27. Unser Wiederherstellungspunkt liegt bei der Protokollierung der Sequenznummer 28.
. Bei der Wiederherstellung von einem Standby-DB-Host setzen Sie als Oracle-Benutzer die Variable ORACLE_HOME auf die aktuelle Oracle-Installation auf dem Standby-DB-Server und ORACLE_SID auf die SID der primären Oracle-Instanz. In diesem Test ist dies NTAP.
+
....
...
....
. Erstellen Sie als oracle-Benutzer eine allgemeine Oracle-Init-Datei im Oracle_HOME/dbs-Verzeichnis mit einer Konfiguration der richtigen Administratorverzeichnisse. Am wichtigsten ist, dass Sie Oracle  `flash recovery area` Verweisen Sie auf den GCNV NFS-Mount-Pfad, wie im primären Oracle VLDB-Server definiert.  `flash recovery area` Die Konfiguration wird in Abschnitt dargestellt `Setup Oracle RMAN incremental merge to image copy on GCNV for in-server VLDB backup`. Legen Sie die Oracle-Steuerdatei auf das GCNV NFS-Sicherungsdateisystem fest.
+
[source, cli]
----
...
....
+
Die oben genannte init-Datei sollte im Falle einer Abweichung durch eine wiederhergestellte Backup-init-Datei vom primären Oracle VLDB-Server ersetzt werden.

. Starten Sie als Oracle-Benutzer RMAN, um die Oracle-Wiederherstellung auf dem primären oder Standby-DB-Server-Host auszuführen. Starten Sie zunächst die Oracle-Instanz in  `nomount` Zustand.
+
....
...
....
. Datenbank-ID festlegen. Die Datenbank-ID kann aus dem Oracle-Dateinamen der Image-Kopie am GCNV NFS-Bereitstellungspunkt abgerufen werden.
+
....
...
....
. Stellen Sie die Init-Datei aus der SP-Datei im Ordner $ORACLE_HOME/dbs wieder her, um die Parameterdatei später so zu aktualisieren, dass sie mit der primären VLDB übereinstimmt.
+
....
...
....
. Überprüfen Sie, ob sich die Datenbankdateien jetzt im Sicherungsdateisystem auf GCNV befinden.
+
....
...
....
. wenigen Bei der Wiederherstellung auf einem Standby-DB-Server muss der Primärspeicher in den Standby-DB-Server importiert werden. Minuten Ausfallzeit möglich.


Damit ist die zeitpunktbezogene Wiederherstellung der Oracle VLDB-Datenbank von der Backup-Image-Kopie auf dem GCNV NFS-Dateisystem auf demselben primären DB-Server oder einem Standby-DB-Server abgeschlossen.

====


=== Klonen Sie eine Oracle VLDB für andere Anwendungsfälle aus der Backup-Image-Kopie

[%collapsible%open]
====
Das Klonen einer neuen Oracle VLDB aus der Backup-Image-Kopie auf GCNV ähnelt der zeitpunktbezogenen Wiederherstellung der Primärdatenbank auf einem neuen DB-Server. Das NetApp Solutions Engineering Team stellt ein Automatisierungs-Toolkit bereit, um die gesamten Prozesse für eine schnelle (Minuten) und mühelose Implementierung zu vereinfachen.

Das folgende Automatisierungs-Toolkit enthält ein Playbook, mit dem Klone einer Oracle VLDB aus einer primären Datenbank-Backup-Image-Kopie erstellt oder aktualisiert werden können, die per NFS-Mount auf einem GCNV-Speicher gespeichert ist, um ein vollständiges Klon-Lebenszyklusmanagement zu ermöglichen. Weitere Einzelheiten finden Sie in der Dokumentation: link:https://docs.netapp.com/us-en/netapp-solutions/databases/automation_ora_bkup_clone.html["Automatisierter Oracle VLDB-Klon-Lebenszyklus mit In-Server-Image-Backup"^] .
