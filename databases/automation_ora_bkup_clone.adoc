...
summary: 'Die Lösung bietet ein Ansible-basiertes Automatisierungs-Toolkit zum Einrichten, Klonen und Aktualisieren einer Oracle-geklonten Very Large Database (VLDB) aus einer Oracle-Datenbank-Image-Sicherungskopie in NetApp-Cloud-Volumes, die über eine öffentliche Cloud-Plattform verfügbar sind. Das Toolkit unterstützt zunächst Google Cloud NetApp Volumes (GCNV), kann aber um weitere Cloud-Volumes wie Azure NetApp Files (ANF) oder Amazon FSx for ONTAP (FSxN) erweitert werden.' 
---
...
[role="lead"]
Die Lösung bietet ein Ansible-basiertes Automatisierungs-Toolkit zum Einrichten, Klonen und Aktualisieren einer Oracle-geklonten Very Large Database (VLDB) aus einer Oracle-Datenbank-Image-Sicherungskopie in NetApp-Cloud-Volumes, die über eine öffentliche Cloud-Plattform verfügbar sind. Das Toolkit unterstützt zunächst Google Cloud NetApp Volumes (GCNV), kann aber um weitere Cloud-Volumes wie Azure NetApp Files (ANF) oder Amazon FSx for ONTAP (FSxN) erweitert werden.



== Zweck

Die Nutzung der Quick-Clone-Funktion (Minuten) von NetApp ONTAP Volumes für Oracle VLDB aus einer serverinternen Datenbank-Image-Backup-Kopie in der Public Cloud dient mehreren Zwecken. Sie bietet eine nahezu in Echtzeit verfügbare Produktionsdatenbank in voller Größe für Reporting-, Entwicklungs- oder UAT-Anwendungsfälle. Dies spart Speicherkosten, insbesondere wenn ein Thin Cloning der primären Datenbank-Backup-Volumes möglich ist. Dieses Ansible-basierte Automatisierungs-Toolkit ermöglicht es Benutzern, geklonte Oracle-Datenbanken nach benutzerdefinierten Zeitplänen einzurichten, zu klonen und zu aktualisieren, um ein optimiertes Lebenszyklusmanagement zu gewährleisten. Das Toolkit ist für den Anwendungsfall von Oracle VLDB in der Google Public Cloud konzipiert und nutzt GCNV als serverinternen Datenbank-Backup-Speicher.

Diese Lösung eignet sich für folgende Anwendungsfälle:

* Richten Sie Datenbankklon-Konfigurationsdateien für das automatisierte Klonen von Oracle-Datenbanken ein.
* Erstellen oder aktualisieren Sie eine geklonte Oracle-Datenbank aus einer Sicherungskopie des primären Datenbank-Images mithilfe eines Ansible-Playbooks auf einem Scheduler.
...
* Ein Anwendungsbesitzer, der Oracle-Datenbanken für andere Anwendungsfälle aus einer Datenbank-Image-Sicherungskopie klonen möchte.
...
NOTE: Der Vereinfachung halber sollte der Oracle-Zielserver für den Klon identisch zum primären Oracle-Server konfiguriert sein, beispielsweise hinsichtlich des Oracle-Software-Stacks und des Verzeichnislayouts für Oracle Home usw.
...
[%collapsible%open]
====
Das Toolkit enthält eine Hostdatei, die die Quelle definiert und auf Oracle-Hosts abzielt, auf die das Ansible-Playbook ausgeführt wird. Normalerweise umfasst es den primären Datenbankserver-Host und den Ziel-Klon-Datenbankserver-Host. Im Folgenden finden Sie eine Beispieldatei. Ein Hosteintrag enthält die IP-Adresse des Zielhosts sowie den SSH-Schlüssel für den Benutzerzugriff auf den Host, um den Klon- oder Aktualisierungsbefehl auszuführen. Auf den Google Cloud NetApp Volumes Storage wird über die gcloud cli zugegriffen und gemanagt.
...
[%collapsible%open]
====
Hostvariablen werden im Verzeichnis Host_VARs mit dem Namen {{ Host_Name }}.yml definiert, das nur für den jeweiligen Host gilt. Für diese Lösung wird nur die DB-Host-Parameterdatei für Zielklone konfiguriert. Die primären Oracle-DB-Parameter werden in der Datei „Globale Variablen“ konfiguriert. Nachfolgend ist ein Beispiel für die Zieldatei orac.yml der Oracle-Clone-DB-Hostvariable orac.yml aufgeführt, die eine typische Konfiguration zeigt.
...
[%collapsible%open]
====
Der Oracle-Zielserver für Clones sollte denselben Oracle-Softwarestack aufweisen wie der Oracle-Quellserver, der installiert und gepatcht ist. Die Variable $ORACLE_HOME sollte idealerweise mit der Einstellung des Quell-Oracle-Servers übereinstimmen. Weicht die Ziel-ORACLE_HOME-Einstellung von der Konfiguration des primären Oracle-Servers ab, erstellen Sie einen symbolischen Link, um die Unterschiede zu umgehen.

Wenn die primäre Datenbank mit ASM konfiguriert ist, kann die primäre Gruppe der Datendateien zur ASM-Gruppe gehören und die gleiche ASM-Gruppe mit der gleichen Gruppen-ID sollte zum Klonhost hinzugefügt werden, um Berechtigungsprobleme zu vermeiden.
...
Um zusätzliche Datenbanken zu klonen, erstellen Sie eine separate Datei „oracle_bkup_clone_n_gcnv.yml“ und „oracle_bkup_clone_n_gcnv.sh“. Konfigurieren Sie die Ansible-Zielhosts, die Datei global vars.yml und hostname.yml im Verzeichnis Host_vars entsprechend.
...
=== Details zur Playbook-Ausführung

[%collapsible%open]
====
Die folgende Protokolldatei erfasst die Details der Ausgabe einer vollständigen Ausführung des Playbooks zu Referenzzwecken.
