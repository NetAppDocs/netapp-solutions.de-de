...
summary: In diesem Abschnitt werden die detaillierten Schritte aufgeführt, die erforderlich sind, um GPFS zu konfigurieren und Daten mithilfe von NetApp XCP in NFS zu verschieben. 
---
...
link:bda-ai-business-benefits.html["Früher: Geschäftsvorteile."]

[role="lead"]
In diesem Abschnitt werden die detaillierten Schritte aufgeführt, die erforderlich sind, um GPFS zu konfigurieren und Daten mithilfe von NetApp XCP in NFS zu verschieben.



== Konfigurieren Sie GPFS

. Laden Sie Spectrum Scale Data Access für Linux auf einem der Server herunter und installieren Sie ihn.
+
....
...
…
....
. Installieren Sie das Voraussetzungspaket (einschließlich Chef und Kernel Header) auf allen Knoten.
+
....
...
....
. Deaktivieren Sie SELinux in allen Knoten.
+
....
...
[root@mastr-51 5.0.3.1]#
....
. Richten Sie den Installations-Node ein.
+
....
...
[root@mastr-51 installer]#
....
. Fügen Sie den Admin-Knoten und den GPFS-Knoten zur Cluster-Definitionsdatei hinzu.
+
....
...
[root@mastr-51 installer]#
....
. Fügen Sie den Manager-Knoten und den GPFS-Knoten hinzu.
+
....
...
[root@mastr-51 installer]#
....
. Fügen Sie die GUI-, Admin- und GPFS-Nodes hinzu.
+
....
...
[root@mastr-51 installer]#
....
. Alle Nodes überprüfen und auflisten.
+
....
...
[root@mastr-51 installer]#
....
. Geben Sie einen Cluster-Namen in der Cluster-Definitionsdatei an.
+
....
...
....
. Geben Sie die Remote-Shell-Binärdatei an, die vom GPFS verwendet werden soll; verwenden Sie `-r argument`.
+
....
...
[root@mastr-51 installer]#
....
. Geben Sie die Binärdatei an, die vom GPFS verwendet werden soll; verwenden `-rc argument`.
+
....
...
[root@mastr-51 installer]#
....
. Geben Sie den für alle GPFS-Knoten zu eingestellten Portbereich an; verwenden Sie `-e argument`.
+
....
...
[root@mastr-51 installer]#
....
. Fügen Sie einen Admin-Node hinzu.
+
....
...
[root@mastr-51 installer]#
....
. Vor der Installation die Konfigurationen überprüfen.
+
....
...
failureGroup=1
....
. Erstellen Sie die NSD-Festplatten.
+
....
...
....
. Überprüfen Sie den Status der NSD-Festplatte.
+
....
...
So exportieren Sie GPFS in NFS:
...
[root@hdp2 ~]#
....
. Die Liste der GPFS-Dateien im angehängten Ordner NFS validieren.
+
....
...
[root@hdp2 gpfstest]#
....
. Verschieben Sie die Daten mit XCP vom GPFS- exportierten NFS zum NetApp NFS.
+
....
...
link:bda-ai-mapr-fs-to-ontap-nfs.html["Nächste: MapR-FS auf ONTAP NFS"]
