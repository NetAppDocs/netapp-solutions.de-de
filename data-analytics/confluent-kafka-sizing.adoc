...
Für Plattform-Teams ohne Anwendungsfälle sollte der Reverse-Modus genutzt werden. Verwenden Sie den Partitions-Modus, um zu berechnen, wie viele Partitionen ein einzelnes Thema benötigt. Siehe https://[] Für die Dimensionierung basierend auf dem Reverse-Modus und den Partitions-Modus.
